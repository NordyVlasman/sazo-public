...
export function ComposerTagPicker(props: Props) {
    const { id, onCommandEnter, disabled } = props
    const { state, dispatch } = useComposer()
    const { tags: selection } = state
        ...
        createTag.mutate(
          { name },
          {
            onSuccess(data) {
              ...
              const newTags = [...selection, option.value];

              dispatch({
                type: ComposerDialogActionType.setTags,
                payload: newTags,
              });
    ...
    function handleChange(newValue: OnChangeValue<Option, true>) {
        ...
        dispatch({ type: ComposerDialogActionType.setTags, payload: newTags })
    }
    ...
    return (
        <CreatableSelect
            ...
            options={tagOptions}
            value={selection.map((name) => ({ value: name, label: name }))}
            instanceId={id}
            ...
            formatCreateLabel={(inputValue) =>
                `Voeg "${formatTagName(inputValue)}" toe`
            }
            styles={customStyles}
        />
    )
